...
| Env | Meaning |
|---|---|
| `ACCOUNT_PASSWORD` | generated by the runtime for THIS run (`crypto/rand`), never stored, never logged. The account's email is `contact_email` from the document — the order's own address, whose inbox the runtime owns |
...
only under the reviewed, pinned commit. And the account is the customer's
booking: the runtime hands them the credentials with their ticket, so they
can check in and manage it themselves.
